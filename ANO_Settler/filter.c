...
void g_v_LoopFilter(s32 *acc_in,s32 *acc_out)
{
	...
	ACC_X_BUF[filter_cnt] = acc_in[A_X];
	ACC_Y_BUF[filter_cnt] = acc_in[A_Y];
	ACC_Z_BUF[filter_cnt] = acc_in[A_Z];
	...
	acc_out[A_X] = temp1 / FILTER_NUM;
	acc_out[A_Y] = temp2 / FILTER_NUM;
	acc_out[A_Z] = temp3 / FILTER_NUM;
	filter_cnt++;
